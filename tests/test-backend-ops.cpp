...
struct test_cpy : public test_case {
    ...
    double max_nmse_err() override {
        return 8e-3; // FIXME: Maybe some bug
    }
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    // test_cases.emplace_back(new test_soft_max(GGML_TYPE_F32, {32, 2, 32, 1}, true,  0.1f, 0.0f)); // Not working in Metalium yet.
    ...
    for (int v : { 0, 1, 2, 3 }) {
        for (int dim : { 0, 1, 2, 3, }) {
            test_cases.emplace_back(new test_concat(GGML_TYPE_F32, {12, 12, 13, 14}, 7, dim, v));
            test_cases.emplace_back(new test_concat(GGML_TYPE_I32, {12, 12, 13, 14}, 7, dim, v));
        }
    }
